...
bool BucatiniHit::operator==(const BucatiniHit& right) const {
...
void BucatiniHit::Print() {
  if (fTimes.size() > 0) {
    G4cout << "========================================" << G4endl;
    G4cout << "Integral: " << std::setw(7) << fIntegral
           << " Toa: " << std::setw(7) << fToa << G4endl;
  }
  for (int i = 0; i < fTimes.size(); ++i) {
    G4cout << "Time: " << std::setw(7) << G4BestUnit(fTimes[i], "Time")
           << " Wavelength: " << std::setw(7)
